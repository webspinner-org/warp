/**
 * POST /admin/email-verify/finish
 *
 * Body: { email, sessionId, code }
 *
 * Validates the 6-digit code against the latest pending row. On
 * success, returns a signed verified-session ticket the caller passes
 * to subsequent operations (e.g. publish-to-registry).
 */
...
export const POST: RequestHandler = async ({ request, cookies, fetch: f }) => {
  ...
  if (!email || !sessionId || !code) {
    throw error(400, 'email, sessionId, code required');
  }
  ...
  return json({ ok: true, ticket: result.ticket, email: result.verifiedEmail });
};
