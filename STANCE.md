...
canon and not the methodology — it is the *stance*: what we are
...
*tell the customer it does the thing, and run back to make it
true* — applies cleanly. The customer is told the thing is real
...
Brand carries the *target*; proof carries the *position*. The
...
its release cadence. This window is precious. While it lasts:
  ...
  - Speed is not in tension with discipline. The discipline is
    *what makes the speed safe* — every commit ships with tests,
...
Each rule above is *cheap to honor today*. Skipping any one of
...
pressure:
  ...
  - We will not build observability beyond the audit chain and
    ...
    multi-host scale.
  - We will not build patron auth as a separate identity provider
    until the first patron Spinner exists.
...
condition reverses — peer Wizard arrives, patron Spinner ships,
...
The stance is for this epoch. The triggers that end the epoch:
  ...
  - **A patron Spinner ships.** The patron Loom surface ends
    the "operator-only" framing. Patron auth becomes load-
    bearing.
...
fires, this file gets updated *first*, then the implementation
shifts.
...
*Updated 2026-05-12. The Wizard is the sole operator. The Cell is
